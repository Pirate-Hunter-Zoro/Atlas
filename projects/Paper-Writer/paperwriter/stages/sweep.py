...
from ..gates import (citations, crossrefs, length, numbers, paragraphs, polarity,
                     procedures, prose, readability, repetition, sentences,
                     terminology, venue)
...
def _packet_scope(out, texts, evidence, lock, references, where):
    ...
    # Sameness asserted in one section and denied in another. Advisory, and the reason
    # is in the gate's own docstring: it cannot see subjects, so a paper that says A
    # and B share a cohort while C and D do not is a pair here and is not a defect.
    # Every document, because a supplement is where two sections most easily disagree
    # — the one that motivated the gate carried the manuscript's contradiction twice.
    for name, text in texts.items():
        for clash in polarity.check(text).clashes:
            _add(out, name, clash.asserted[0][0], "polarity", "advisory",
                 clash.reason(), clash.denied[0][1])
