...
### The claim the paper makes and then unmakes

Every gate above measures a sentence, a paragraph, or a count. None of them reads two
sentences and notices that the paper has contradicted itself, and `repetition` is the
near miss: it reports a point made three times and forgives a point made twice, which
is exactly the count a contradiction has.

A Methods section said, in bold, that **both representations were built from the same
curated field inventory**. Twenty-five lines later, the same section said that FEATURE
and EMBEDDED **do not receive an identical field inventory**. Both sentences were true.
The first was about which fields predictor selection chose; the second was about how
some of those chosen fields were then encoded. Neither sentence said which sense it
meant. A reader hit the second one, went back to the first, and stopped trusting the
paragraph — the one-read rule failing at a distance no per-sentence check can see.

Every gate passed it. The sentences were short, the paragraphs opened on their claims,
the vocabulary was locked, every number was in the ledger, and the two halves of the
contradiction were a page apart in one section.

So `polarity` collects the sentences that assert two things are *the same* or
*identical* in some respect, keys each on the head noun of that respect, and reports
any noun the document both asserts and denies. Three refinements came out of pointing
it at a real packet. Polarity is read at the **clause**, so "built from the same
inventory, and neither was built from a raw record" is one assertion rather than a
contradiction of itself. A **conditional** is neither side of a contradiction — the
supplement section that introduces the crosswalk says the comparison holds *to the
extent that* both encodings carry the same information, and the table under it is the
paper checking that. And an **of-phrase keys on the thing rather than the container**,
because "the same set of values" is a claim about values and "an identical set of
rendered fields" is a claim about fields; keying both on "set" made two unrelated
sentences a pair.

It is **advisory**, and the reason is that it cannot see subjects. A paper may
legitimately say that two arms share a cohort while two others do not, and that is a
pair here and not a defect. So the finding is a pair to be read rather than a defect to
be fixed — and when there is a defect, the fix is usually one word, because the two
sentences are about different senses of one noun and neither one names its sense.

Pointed at the finished manuscript and its supplement, it found that contradiction in
the manuscript and the same one again in the supplement, where S8 said both encodings
are given the same fields and M5 said they are not.
