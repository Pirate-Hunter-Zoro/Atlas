		...
		if sols[r1][c1][r2] == -2 {
			...
			if grid[r1][c1] == -1 || grid[r2][c2] == -1 {
				...
				if r1 == r-1 && r2 == r-1 && c1 == c-1 && c2 == c-1 {
					...
					initial_amount := grid[r1][c1]
					if r1 != r2 || c1 != c2 {
						initial_amount += grid[r2][c2]
					}
					// Try both right
					if solve(r1, c1+1, r2) != -1 {
						record = max(record, initial_amount + solve(r1, c1+1, r2))
					}
					// Try both down
					if solve(r1+1, c1, r2+1) != -1 {
						record = max(record, initial_amount + solve(r1+1, c1, r2+1))
					}
					// Try first right, second down
					if solve(r1, c1+1, r2+1) != -1 {
						record = max(record, initial_amount + solve(r1, c1+1, r2+1))
					}
					// Try first down, second right
					if solve(r1+1, c1, r2) != -1 {
						record = max(record, initial_amount + solve(r1+1, c1, r2))
					}
...
The graph is given as a 2D array of edges where edges[i] = [u_i, v_i, cnt_i] indicates that there is an edge between nodes ui and vi in the original graph, and cnt_i is the total number of new nodes that you will subdivide the edge into. Note that cnt_i == 0 means you will not subdivide the edge.

To subdivide the edge [u_i, v_i], replace it with (cnt_i + 1) new edges and cnt_i new nodes. The new nodes are x1, x2, ..., xcnt_i, and the new edges are [u_i, x1], [x1, x2], [x2, x3], ..., [xcnt_{i-1}, xcnt_i], [xcnt_i, v_i].
