...
- Stated in their own words on `t0004-r8`: *order does not matter to preserve the `E↔A`
  relationship; it just changes the result you'll get.* Correct on both halves, affirmed in
  card 0012, with the one consequence added there: they cannot sort `nums` to recover the
  `2^k − 1` shortcut, because the order is the problem.
...
Card 0011 went out and was not answered — `t0004-r8` was a revision of the previous page adding a
conclusion, not an attempt at it — so card 0012 re-poses the same question in full. It is the
first rung of `A(g)`: with the multiples of 2 in position order
