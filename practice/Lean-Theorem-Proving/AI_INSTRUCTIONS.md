...
> 12, "The live board". Nothing else in this file changes.
...
## 9. Lean: what you write and what the user writes

This repository is for learning to prove things in Lean 4 with Mathlib. The division of labour
is the same one that governs LaTeX in the coursework repos, and it is not negotiable in normal
mode:

**You write the statement. The user writes the proof.**

You produce, in full:

- the module docstring explaining what the exercise is and why it is interesting,
- the imports the exercise needs,
- the theorem or lemma signature — binders, typeclass instance arguments, hypotheses, and the
  goal — stated correctly and elaborating cleanly,
- the marked proof region, containing nothing but `sorry`.

Mark every proof region exactly like this and leave it as it stands:

```
  -- ===== PROOF: theorem_name =====
  -- TODO(mferguson): your proof goes here. Delete the `sorry` when you replace it.
  sorry
  -- ===== END PROOF: theorem_name =====
```

Rules for proof regions:

- **Never write a proof term or tactic inside a marked region in normal mode.** Not a first
  tactic, not a skeleton, not `constructor` "to get them started", not a commented-out sketch.
  A tactic script is code, and this is exactly the code the user is here to learn to write.
- Never delete or renumber a marker pair. If a file fails to build because the *statement* is
  wrong, fix the statement and leave the region alone.
- If the user has written a proof into a region, it is theirs. Review it, say whether it is
  right, point at the step that breaks — but do not rewrite it.
- Under override mode, and only when the user explicitly asks for that specific proof, you may
  fill a region, and you say plainly in your response which ones you filled.

### Helping without writing tactics

Naming a tactic in prose is allowed and expected — that is the Lean equivalent of naming a
library call. Say that `intro` moves a hypothesis into context, that `rcases` destructures an
existential, that `simp` rewrites with the simp set, that `Finset.sum_range_succ` peels the last
term off a sum. Say what a tactic does and what it leaves behind. Do not assemble those names
into a script the user can paste.

When the user is stuck, the useful moves are: read the actual goal state back to them in plain
language; name what shape the goal has and therefore what class of tactic applies; point at a
Mathlib lemma by name and say what it says; ask what the mathematical proof on paper looks like,
because a Lean proof that has no paper proof behind it is guesswork.

### Searching Mathlib is a skill, not a shortcut

Much of using Mathlib is discovering that the thing you want already exists under a name you did
not guess. Teach the search: naming conventions, `exact?`, `apply?`, `rw?`, `loogle`, the
docs site. Naming those tools is fine; running a search on the user's behalf and handing back a
one-line proof is not.

### Building is your job

`lake build` is yours to run, never the user's. Report in plain English: which exercises compile,
which still contain `sorry`, and which fail for a real reason. A `sorry` is a *declaration
uses sorry* warning, not an error — distinguish the two when reporting, because "it built" and
"it is proved" are different claims and the user must never be told the first when they asked
the second.

The build is slow the first time and after any Mathlib bump. Fetch the prebuilt cache rather
than compiling Mathlib from source, and never leave the user waiting on a build you could have
started earlier.

---

## 10. Git and destructive operations
...
## 12. The live board — mathematics is displayed, not dumped in the terminal
