...
course. There is no textbook here: the instructor's Mathematica notebooks are the material,
and the user's notes and homework are built from them. The user is the mathematician.
...
**Mathematica is code.** In normal mode you do not write it — not a cell, not a one-liner, not
a "just evaluate this". You name the built-in function, say what each argument is and what it
returns, and the user types it. The `mathematica` environment in the templates is a region the
user fills, exactly like a solution region.
...
3. You review both under section 6.
4. Once the mathematics is settled, you generate or update the `.tex` scaffold under section 7.
5. The user types the mathematics into the marked regions.
