...
\end{problem}
% ===== SOLUTION 22 =====
Induction on $k$. Let $N_j$ be the number of trials until some outcome first occurs $j$
consecutive times, so $N = N_k$.

\emph{Base case, $k = 1$.} One trial always produces a run of length $1$, and
$\frac{m^1 - 1}{m - 1} = 1$.

\emph{Induction step.} Suppose $\EE{N_n} = \frac{m^n - 1}{m - 1}$, and write
$\mu = \EE{N_{n+1}}$. Condition on the trial immediately after the run first reaches length
$n$, using $\EE{X} = \sum_i \EEc{X}{A_i} P(A_i)$ with the binary partition $A_1 = \{$that
trial matches$\}$, $A_2 = \{$it does not$\}$. A match has probability $\frac{1}{m}$ and costs
one guaranteed extra trial, finishing the job. A mismatch has probability
$\frac{m - 1}{m}$, and the mismatched trial itself starts a fresh run of length $1$, so from
there the trials still required are distributed as $N_{n+1}$ counted from the start. Hence
\[
  \mu = \left( \frac{m^n - 1}{m - 1} + 1 \right) \frac{1}{m}
      + \left( \frac{m^n - 1}{m - 1} + \mu \right) \frac{m - 1}{m}.
\]
Put the first bracket over $m - 1$ and move the $\mu$ term to the left, noting that the two
copies of $m - 1$ in $\frac{m^n - 1}{m - 1} \cdot \frac{m - 1}{m}$ cancel:
\[
  \frac{m^n + m - 2 + (m - 1)(m^n - 1)}{m(m - 1)}
    = \mu \left( 1 - \frac{m - 1}{m} \right)
    = \mu \cdot \frac{1}{m}.
\]
Multiplying by $m$ clears the outer denominator, and grouping the two multiples of
$m^n - 1$ gives $(m^n - 1)(m - 1 + 1) = m(m^n - 1)$:
\[
  \mu = \frac{(m^n - 1)(m - 1 + 1) + m - 1}{m - 1}
      = \frac{m^{n+1} - m + m - 1}{m - 1}
      = \frac{m^{n+1} - 1}{m - 1}.
\]
That is the claim for $k = n + 1$, so by induction $\EE{N} = \frac{m^k - 1}{m - 1}$ for every
$k \ge 1$. \qedsymbol
% ===== END SOLUTION 22 =====
