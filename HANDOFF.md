...
- `bash board/test/all.sh` — 97 suites, about twelve minutes. Green before and
  after.
...
- **`board see <path>` IS THE VISION PATH, AND IT REFUSES A FENCE FIRST.** A
  ...
  **And the audit could not have seen this key.** `ai-config`'s
  ...
  read a leaked one straight past. A `deepseek api key` row is in that file and
  was checked against the real key: the old row misses it, the new one catches
  it.
